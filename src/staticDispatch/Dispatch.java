...
public class Dispatch {
	...
	class Car extends Thread {
		...
		int completeAreaNum=0; //完成的区域数，计算随机成本时用到
		// 构造函数 除了这两个变量外其他均为常量
		...
		int findRNextArea()
		{
			if(completeAreaNum>=areaNum/carNum)
				return -2;
			int perNum = areaNum/carNum; //每辆车要走的区域数
			int nextArea= randomRoute[id*perNum+completeAreaNum];
			return nextArea;
		}
...
//			System.out.println("收益："+ 0.47*Math.abs(rr)+" 月卡损失:"+monthCard+" 距离代价:"+dc);
	...
	class Area {
		...
		int redBike = 0;
		// 构造函数
		Area(int id ,String name, int dispatch,int currentDispatch) {
			this.id = id;
		...
		int getDemand(int time) {
			int realNeed = areaNeed[id] - realDispatch;
			return realNeed;
		}
	}
	...
	class Operation {
		...
		public String toString() {
			return "Operation [车编号=" + carId + ", 区域编号=" + areaId + ", 需求=" + currentNeed + ", 满足="
					+ place + ", 完成时间=" + time + "]";
		}
	...
	public int areaNum;
	public static List<Area> areas = new ArrayList<Area>(); // 区域数组
	...
	public boolean [] visited; //调度点i是否被访问过
	public int [] randomRoute;  //生成的随机路径
	...
	public double countCost()
	{
		...
		totalCost = distanceCost + timeCost + punishCost;
		System.out.println("行驶成本="+distanceCost+" 时间成本="+timeCost+" 惩罚成本="+punishCost+ " 总成本="+totalCost);
		return totalCost;
	}
	
	// 生成随机排列
	public static int[] generateRandomRoute(int num)
	{
		int [] arr = new int[num];
	...
	public static void main(String args[]) throws FileNotFoundException, IOException {
		...
		Dispatch.carNum = 2;
		int initCarNum = 0;
		areas.add(dispatch.new Area(0, "远洋未来汇购物中心", 3,3));
		areas.add(dispatch.new Area(1, "北京市朝阳区西柳巷98号", 10,10));
		areas.add(dispatch.new Area(2, "国粹苑C座", -4,-4));
		areas.add(dispatch.new Area(3, "呼家楼地铁站", -13,-13));
		areas.add(dispatch.new Area(4, "八里庄路（汉庭酒店）", -1,-1));
		areas.add(dispatch.new Area(5, "朝阳区定福庄第一小学", 29,29));
		areas.add(dispatch.new Area(6, "红领巾公园", 5,5));
		areas.add(dispatch.new Area(7, "芳菁院小区", 5,5));
		areas.add(dispatch.new Area(8, "高碑店新村小区", -5,-5));
		areas.add(dispatch.new Area(9, "平房区小街巷", -15,-15));
		areas.add(dispatch.new Area(10, "人民日报社", -16,-16));
		areas.add(dispatch.new Area(11, "首都经济贸易大学", 7,7));
		dispatch.initNeed();
		dispatch.visited = new boolean[dispatch.areaNum];
		for(int j=0;j<dispatch.areaNum;j++)
			dispatch.visited[j] = false;
		dispatch.randomRoute = generateRandomRoute(dispatch.areaNum);
//		System.out.print("随机解:");
//		for(int i=0;i<dispatch.areaNum;i++)
//			System.out.print(" "+dispatch.randomRoute[i]);
//		System.out.println();
		// 对车初始化
