...
//			}
	...
	class Operation {
		...
		public String toString() {
			return "Operation [车编号=" + carId + ", 区域编号=" + areaId + ", 需求(不含红包车已满足的)=" + currentNeed +", 红包车满足需求数="+ areas.get(areaId).redBikeNeed+ ", 调度="
					+ place + ", 完成时间=" + time + "]";
		}
	...
	public double inspireMoney = 0.0; //当前激励率下需要的激励金额
	...
	public double countCost()
	{
		...
		for(Area area:areas)
		{
			if(area.redBikeNeed>0)
			{
				...
				double onceCost = area.redBikeNeed * inspireMoney * 0.5;  //更改计算激励成本的规则
	...
	public static void runWithRedBike(double rate)
	{
		...
		dispatch.inspireMoney = UserUtil.getInspireMoney(dispatch.stimulateRate);
		System.out.println("激励率="+dispatch.stimulateRate+"时， 激励金额="+dispatch.inspireMoney);
		dispatch.visited = new boolean[dispatch.areaNum];
		for(int j=0;j<dispatch.areaNum;j++)
			dispatch.visited[j] = false;
		dispatch.areas.add(dispatch.new Area(0, "远洋未来汇购物中心", 3,3,77));
		dispatch.areas.add(dispatch.new Area(1, "北京市朝阳区西柳巷98号", 10,10,116));
		dispatch.areas.add(dispatch.new Area(2, "国粹苑C座", -4,-4,111));
		dispatch.areas.add(dispatch.new Area(3, "呼家楼地铁站", -13,-13,80));
		dispatch.areas.add(dispatch.new Area(4, "八里庄路（汉庭酒店）", -1,-1,75));
		dispatch.areas.add(dispatch.new Area(5, "朝阳区定福庄第一小学", 29,29,57));
		dispatch.areas.add(dispatch.new Area(6, "红领巾公园", 5,5,58));
		dispatch.areas.add(dispatch.new Area(7, "芳菁院小区", 5,5,51));
		dispatch.areas.add(dispatch.new Area(8, "高碑店新村小区", -5,-5,55));
		dispatch.areas.add(dispatch.new Area(9, "平房区小街巷", -15,-15,141));
		dispatch.areas.add(dispatch.new Area(10, "人民日报社", -16,-16,87));
		dispatch.areas.add(dispatch.new Area(11, "首都经济贸易大学", 7,7,47));
		dispatch.initNeed();
		// 对车初始化
